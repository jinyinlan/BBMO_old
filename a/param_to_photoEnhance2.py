...
def param_to_photoEnhance(parameter, paramfile_name, dirName, mode):
	dataset_image_dir = '/home/omiya/data/flickr_teacherData0319/'
	dirName = dirName + '/'

	memoFileName = dirName+paramfile_name.split('.')[0]+'_parameter_cnn.txt'
	make_parameter_file(parameter, memoFileName) #ndarray params -> .txt memo

	XMPdataFile_name = dirName + paramfile_name.split('.')[0] + '_XMPdata_cnn.txt'
	encodeF2B.encode_f2b(memoFileName, XMPdataFile_name) #.txt memo -> binary

	#input_imX = dataset_image_dir + mode + '/' + paramfile_name + '-s.png'
	if mode == 'val':
		input_imX = dataset_image_dir + mode + '/' + paramfile_name
	elif mode == 'test':
		input_imX = dataset_image_dir +  paramfile_name

	print(input_imX)
	
	xmpfile_name = dirName + paramfile_name.split('.')[0] + '_cnn.xmp'
	makeXML.makeXMP_file(input_imX, XMPdataFile_name, xmpfile_name) # binary -> xmp file

	#enhance photo
	output_im = dirName + paramfile_name.split('.')[0] + '_cnn.jpg'
...
if __name__ == '__main__':
	...
	dirName = '/home/omiya/data/result0312/'
	print (descaled_params)

	param_to_photoEnhance(descaled_params, paramfile_name, dirName, mode)
